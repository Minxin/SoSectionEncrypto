package com.fancy.encryptoso;

import com.fancy.encryptoso.ElfType32.Elf32_Sym;
import com.fancy.encryptoso.ElfType32.elf32_phdr;
import com.fancy.encryptoso.ElfType32.elf32_shdr;
...
public class EncodeSection {
	...
	public static void main(String[] args){
		...
		byte[] fileByteArys = Utils.readFile("so/libencrypto.so");
		...
		Utils.saveFile("so/eclibencrypto.so", fileByteArys);
	...
	private static void encodeSection(byte[] fileByteArys){
		...
		*/
		...
		/*for fix error '.dynamic section header was not found'*/
		int nSize = mySectionSize/4096 + (mySectionSize%4096 == 0 ? 0 : 1);
		byte[] entry = new byte[4];
		entry = Utils.int2Byte((mySectionSize<<24) +(mySectionOffset<<8)+ nSize);
		Utils.replaceByteAry(fileByteArys, 24, entry);
